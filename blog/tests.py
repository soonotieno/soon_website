...
class TestView(TestCase):
    ...
    def test_post_detail(self):
        ...
        login_success = self.client.login(username='smith', password='nopassword')  # log in을 한 경우에는
        ...
        # 다른 사람인 경우에는 없다.
        login_success = self.client.login(username='obama', password='nopassword')  # log in을 한 경우에는
        self.assertTrue(login_success)
        response = self.client.get(post_000_url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_comment(self):
        ...
        comment_000 = create_comment(post_000, text='I am president of the America', author=self.user_obama)
        comment_001 = create_comment(post_000, text='a test comment', author=self.author_000)
        ...
        response = self.client.post(
            ...
            {'text': 'I am Not president of the America anymore'},
            follow=True
        )
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
        self.assertNotIn('I am president of the America', soup.body.text)
        self.assertIn('I am Not president of the America anymore', soup.body.text)

    def test_search(self):
        post_000 = create_post(
            title='Stay Fool, Stay Hungry',
            content='Amazing AppleS Story',
            author=self.author_000,
        )
